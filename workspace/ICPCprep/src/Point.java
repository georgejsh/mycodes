...
class Circle {
	...
	Point[] lineinter(Circle C,Line L) {
		Point imc= L.findimage(L, C.c);
		if((new Point()).distance(imc, C.c)>2*C.r)
			return null;	
		Point res[] = new Point[2],temp1=new Point(),temp2=new Point();
		res[0]=temp1;
		res[1]=temp2;
		if(L.b==0.0) {
			temp1.x=temp2.x=L.c;
			temp1.y=C.c.y+Math.sqrt(C.r*C.r-(L.c-C.c.x)*(L.c-C.c.x));
			temp2.y=C.c.y-Math.sqrt(C.r*C.r-(L.c-C.c.x)*(L.c-C.c.x));
		}
		else {
			Double temp=L.c+C.c.y;
			Double quada=L.a*L.a+1,quadb=-2*C.c.x+2*L.a*temp,quadc=C.c.x*C.c.x+temp*temp-C.r*C.r;
			Double x1=-quadb+Math.sqrt(quadb*quadb-4*quada*quadc)/2/quada;
			Double x2=-quadb-Math.sqrt(quadb*quadb-4*quada*quadc)/2/quada;
			temp1=new Point(x1,-L.a*x1-L.c);
			temp2=new Point(x2,-L.a*x2-L.c);
		}
		return res;
	}
	//circles with velocity
	double findmaxarea(Circle C1,Circle C2,Point V1,Point V2) {
